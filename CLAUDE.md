...
| `MESH_QA_CONTEXT_BUDGET` | `12000` | Char budget for the Q&A context pack (Phase 21); lower-ranked claims beyond it are dropped (logged) |
| `MESH_QA_RELEVANCE_FLOOR` | `1e-6` | Minimum `ts_rank` for a retrieved row to enter the context pack |
| `MESH_ASK_AGENT_URLS` | `http://research-qa:8016` | API → ResearchQA agent base URL(s) for `POST /api/v1/ask` |
| `MESH_ASK_TIMEOUT` | `120` | `POST /api/v1/ask` A2A skill timeout (seconds) |
| `MESH_LLM_MODEL_RESEARCH_QA` | (unset) | Static model pin for the Q&A agent; when set, always wins and disables routing |
| `MESH_LLM_MODEL_RESEARCH_QA_STRONG` | (falls back to `MESH_ROUTE_STRONG_MODEL`) | Strong-tier model for the Q&A agent when routing is enabled |
