...
the same CLI entry point a human would run (``mesh-pipeline``,
``mesh-skeptic-sweep``). All DB writes happen on the coordinator/sweep
side, exactly as when the user runs ``make pipeline`` by hand.
