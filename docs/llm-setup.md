...
A typical `make controller-apply` run (a round of ~20 arxiv papers, ~500 input + ~500 output tokens each) costs **roughly $0.01–$0.05**. Set a low monthly cap in the console for peace of mind.
...
3. `make controller-apply` — the controller's `make_llm_client()` will instantiate the new backend.
...
make controller-apply   # extraction uses Haiku; the skeptic skill uses Opus
```
