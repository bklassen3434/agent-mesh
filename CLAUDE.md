...
- **9** — Wiki redesign. Knowledge sections live under `/knowledge/*`. (The Phase 9 schedule-control "Pipelines" page + the `schedules` table and standalone scheduler were **removed** once the self-driving controller became the sole orchestrator — see the controller-only bullet below.)
...
- **Controller-only orchestration** — The deterministic **controller** (`mesh-controller`) is now the **sole** orchestration job (seeded enabled; the old fixed `ingest`/`skeptic`/`discovery` jobs and the standalone consolidation sweeps are deleted). It senses the field into `Tension`s and runs the whole loop — scout → extract → resolve → synthesize → challenge → investigate — under an explicit **rule engine** (`mesh_agents.rules`), dispatching up to `MESH_CONTROLLER_STEP_CAP` activations per round (to quiescence) and escalating a stalled tension to a swarm after `MESH_CONTROLLER_ESCALATE_AFTER`. Belief decay/archival and memory consolidation are folded in as **cooldown-gated rules** (`aging_belief`→`maintain-belief`, `consolidatable_memory`→`consolidate-memory`, fired on a timer like scouting; `MESH_CONTROLLER_MAINTAIN_COOLDOWN_SEC`). **Self-improvement** rides the same primitive: a cooldown-gated `evaluate_accuracy`→`sense-accuracy` rule web-grounds a belief sample and **accumulates** append-only fault-attribution *concerns* (`agents.improvement_concerns`) — attributing each accuracy/freshness failure to a component (extraction/synthesis/freshness/coverage/confidence) — **without editing anything**; once open concerns for a component cross a threshold, a *derived* `improvable_component`→`improve-component` tension drafts a candidate (the accuracy gradient seeds the prompt optimizer, frozen-set A/B as a cheap pre-filter) and **opens a shadow experiment** rather than installing: `advance-experiment` runs the candidate *beside* the live pipeline on real sources (output quarantined, never in the KB), grades both arms, and promotes into `catalog.prompt_versions` only once it beats control over a window. The eval grades the **whole belief corpus** on a rolling least-recently-graded cursor (`agents.belief_grades` = coverage + accuracy time-series), and attribution spans the **whole pipeline** (scout→extract→resolve→synthesize→challenge→confidence→decay), not just prompts. Accumulate → threshold → shadow A/B → windowed promote, no schedule. Fixing a stage is a pluggable **actuator** (`mesh_agents/actuators/`: `draft`→`shadow_sample`→`promote_effects`; the improve/advance skills are generic over the registry) in one of three kinds — **prompt A/B** (extraction: candidate extract-source prompt → `prompt_versions`), **config A/B** (confidence: candidate evidence weights → recompute calibration over the grade ledger → `field_config` the live `confidence_fn` overlays), or **action** (freshness/coverage → route to scout/investigate). Attribution spans the whole pipeline so every stage's concerns accumulate; a stage auto-fixes once its actuator is registered. Wired today: `extraction` (prompt) + `challenge` (prompt: candidate skeptic prompt replayed on graded beliefs, scored on whether it flags the web-wrong ones without false alarms → `challenge-belief` `prompt_versions` the live `resolve_skeptic_system` overlays) + `confidence` (config) + `entity_resolution` (config: candidate merge threshold graded against the LLM adjudicator-as-oracle over real candidate pairs → `field_config` the live `ResolutionConfig.resolve` overlays) + `decay` (config: candidate half-life → recompute each graded belief's decayed confidence → calibration vs web verdict → `field_config` the live `DecayConfig.resolve` overlays); `freshness`/`coverage` route to scout/investigate as safe actions. `synthesis` is **not** actuatable yet — the `synthesize-belief` stage is deterministic (no prompt to A/B); making it improvable requires first giving it an LLM-authored belief statement. See `docs/self-improvement.md`.) The LLM **skills are the agentic unit** (prompt + LLM + structured output + injected memory); `ClaimExtractorAgent`/`SkepticAgent` are now thin adapters for the orphaned A2A servers in `apps/agents`. The controller is **self-driving**: `mesh-controller --apply --forever` runs it as an always-on daemon (the default-on `controller` compose service) that repeats the full pass and idles `MESH_CONTROLLER_IDLE_SLEEP_SEC` between empty passes — **no external scheduler/cron**. All cadence comes from the rules' own cooldowns (scouting, maintenance) plus that idle backoff. The old `scheduler` service, the `schedules` table, and the wiki **Pipelines** page have been **deleted** (the controller is the only orchestrator). The CLI `discover` (read-only preview) and `consolidate-beliefs` (one-time backfill) remain. See `docs/deterministic-controller.md` (skill coverage in `docs/agentic-status.md`).
...
- **`apps/api`** (Phase 3) — FastAPI HTTP service on :8000. One pooled read-only Postgres connection (the `mesh_reader` role) per request via a FastAPI dependency. Endpoints under `/api/v1/`; OpenAPI at `/openapi.json`, Swagger UI at `/docs`. Best-effort idempotent schema-ensure at startup (no-op unless given an owner DSN); all request handling is read-only and enforced by the reader role's grants. The few operational writes are the Ask `POST` and the Fields/Connectors `PATCH`/`PUT`; `GET /api/v1/graph/data` is pre-aggregated (top-200 nodes by belief count). CORS allows POST/PATCH/PUT from the wiki origin.
- **`apps/wiki`** (Phase 3, redesigned Phase 9) — Next.js 15 App Router wiki on :3000. Mostly server components; interactive bits (nav dropdown/drawer, graph) are client components built on Radix-based shadcn primitives in `src/components/ui/`. Nav: `Daily Brief | Ask | Knowledge ▾ | Graph | Agents | Fields | Connectors`; knowledge sections live under `/knowledge/*` with `next.config` redirects from old paths. TypeScript types live in `apps/wiki/src/lib/api-types.ts`, generated from the API's OpenAPI spec by `openapi-typescript` (`make types`). CI regenerates and diffs to detect drift.
...
| `LANGGRAPH_POSTGRES_URL` | (empty) | DSN for the single Postgres: LangGraph checkpoints (in `public`) and the `knowledge` schema. Also the base DSN that `MESH_PG_*` fall back to. Unset → in-memory checkpointer (local/tests) |
| `LANGGRAPH_POSTGRES_PASSWORD` | `langgraph` | Password for the `mesh-postgres` container |
| `NEXT_PUBLIC_LANGFUSE_URL` | (empty) | Optional; when set, run-detail views link to Langfuse |
...
| `MESH_CONTROLLER_EVAL_COOLDOWN_SEC` | `259200` (3 days) | Min seconds between web-grounded accuracy evaluations of a field. The cooldown-gated `evaluate_accuracy` tension → `sense-accuracy` skill grades a belief sample against the web, measures freshness, and **only accumulates fault-attribution concerns** (`agents.improvement_concerns`) — it never edits the system. Longer than routine maintenance because the eval is expensive |
| `MESH_EVAL_SAMPLE_SIZE` | `15` | Held beliefs the `sense-accuracy` skill grades per eval pass |
| `MESH_EVAL_JUDGE_MODEL` | (judge default) | Anthropic model the web-search accuracy judge uses (needs `ANTHROPIC_API_KEY`; the skill degrades to no-op without it) |
| `MESH_IMPROVE_CONCERN_THRESHOLD` | `5` | Open fault-attribution concerns for one component before the derived `improvable_component` tension fires an A/B improve pass — the accumulate-then-activate threshold (like every other tension). Only components with a wired A/B actuator (today: `extraction` → the extract-source prompt) auto-fire; others accumulate visibly |
| `MESH_IMPROVE_SEVERITY_THRESHOLD` | `3.0` | Alternative trigger: summed concern severity for a component past which it activates (OR'd with the count threshold) |
| `MESH_EXPERIMENT_MIN_SAMPLE` | `20` | Per-arm graded samples a **shadow A/B** needs before it may be decided. A promoted concern-fix isn't installed directly — `improve-component` opens a shadow experiment (`agents.improvement_experiments`); `advance-experiment` runs the candidate *beside* prod on real sources (its output quarantined, never in the KB), grades both arms, and promotes only once treatment beats control by the margin over the window |
| `MESH_EXPERIMENT_MARGIN` | `0.02` | Min accuracy margin (treatment − control) for a shadow A/B to promote |
| `MESH_CONTROLLER_EXPERIMENT_COOLDOWN_SEC` | `3600` | Min seconds between shadow-eval sampling batches for one experiment (each batch re-runs both prompt arms on real sources — LLM-heavy; the decision itself isn't paced) |
| `MESH_EXPERIMENT_SAMPLE_BATCH` | `5` | Sources scored per shadow-eval batch (both arms each) |
