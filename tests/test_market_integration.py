"""End-to-end check of the agentic market with the *real* skills registered.

Where the per-skill tests drive one skill in isolation and ``test_market.py``
drives the loop with a fake skill, this test wires the whole machine together:
it seeds a small board, registers the actual Phase-2 skills via
``load_builtin_skills`` (the production startup path), and runs ``run_market``.

The board is seeded so the agenda surfaces one tension for each of the three
things the migration doc calls out — an unread source, a thin belief, and a
duplicate-looking entity pair — which between them exercise every skill class:

* ``unextracted_source``    → ``extract-source``   (funded; no LLM here → no effects)
* ``thin_belief`` / ``under_evidenced_entity`` → ``investigate-gap`` (opens investigations)
* ``merge_candidate``       → ``merge-candidate``  (high band, no LLM → a merge effect)
* (a claimless held belief may also surface ``stale_belief`` → ``challenge-belief``)

No LLM is reachable in the test environment (the api key is removed), so the
LLM-bound skills degrade exactly as they do in production — caught by the
market's per-skill guard or by their own fallbacks — and the *rule-based* paths
(``merge-candidate`` high band, ``investigate-gap`` fallback) still produce real
effects. The headline assertions are the market-level ones: every tension is
*handled* by some registered skill (nothing is ``skipped_no_skill``), the market
*funds* that work, and it *produces effects* — first in shadow (previewed, never
written), then live (materialised through the write gateway).
"""
...
# The five Phase-2 skill modules load_builtin_skills() imports. We reload them on
# a clean registry so their @register_skill decorators re-run deterministically
# (the registry is global state another test may have cleared; importing an
# already-imported module won't re-register — see test_skill_extract_source).
_SKILL_MODULES = (
...
def real_skills(monkeypatch: pytest.MonkeyPatch) -> Iterator[list[str]]:
    ...
    try:
        ...
        # Leave the registry populated with a clean re-registration for later tests.
...
def _seed_board(conn: MeshConnection) -> dict[str, Any]:
    ...
    source = Source(
        ...
        raw_content_hash="hash-market-integration",
    )
    create_source(conn, source, field_id=DEFAULT_FIELD_ID)

    # 2. A thin belief — held, resting on no supporters (diversity 0, supporters 0).
    belief = Belief(
    ...
    # 3. A duplicate-looking entity pair — same type, identical name embeddings.
...
def test_market_shadow_funds_and_handles_every_tension(
    real_skills: list[str], tmp_db: MeshConnection
) -> None:
    """Shadow round: the real skills cover every tension, the market funds the
    work and previews effects — and writes nothing."""
    # Sanity: the production startup path registered the full fleet.
    assert {
        "extract-source",
        "merge-candidate",
        "synthesize-belief",
    ...
    result = asyncio.run(run_market(DEFAULT_FIELD_SLUG, shadow=True, conn=tmp_db))

    # Shadow previews exactly one round.
    assert len(result.rounds) == 1
    r0 = result.rounds[0]

    # The board surfaced our three seeds (+ the two under-evidenced dup entities).
    assert r0.candidates >= 3
    # Every tension is handled by a registered skill — nothing falls through.
    assert r0.skipped_no_skill == 0
    # The market funded work and the funded skills produced effects to preview.
    assert r0.funded == r0.candidates
    assert r0.funded >= 3
    assert r0.effects >= 1
...
def test_market_live_materialises_effects_through_gateway(
    real_skills: list[str], tmp_db: MeshConnection
) -> None:
    ...
    result = asyncio.run(
        run_market(DEFAULT_FIELD_SLUG, shadow=False, max_rounds=1, conn=tmp_db)
    )
