"""LEGACY imperative A2A coordinator (pre-Phase-8).

Kept runnable for one transition commit via ``mesh-pipeline --a2a --legacy``
so the LangGraph coordinator can be compared against the old fixed DAG.
Removed once the LangGraph graph is verified (Phase 8 step 6).
...
# Reuse the canonical result type + parser so this legacy entry point shares a
# return type with the LangGraph coordinator (keeps mesh-pipeline __main__'s
# conditional import type-clean during the one-commit transition). parse_since
# is re-exported (as-alias) for mesh-pipeline __main__'s --legacy branch.
from mesh_pipeline.coordinator import PipelineResult
from mesh_pipeline.coordinator import parse_since as parse_since
